...
impl FrameRenderer {
    ...
    fn render_background_pixel(
        ...
        let tile_column = x_in_name_table / 8;
        let tile_row = y_in_name_table / 8;
        let nt_idx = tile_row * 32 + tile_column;
        let nametable = if is_second_nametable {
            second_nametable
        } else {
            main_nametable
        };
